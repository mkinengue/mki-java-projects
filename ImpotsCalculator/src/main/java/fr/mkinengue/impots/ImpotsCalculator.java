...
public class ImpotsCalculator {
    ...
    /** Contient pour une année donnée les tranche min, max d'une tra nche d'impôt donnée ainsi que le taux d'imposition pour cette tranche */
     ...
     * @return float
     */
    public static float getImpotsValue(final int annee, final float revenu, final float partFiscale, final MultiKeyMap bareme) {
        if (annee < 2008 || annee > 2012) {
            if (bareme == null) {
                throw new RuntimeException("Le barême pour l'année " + annee + " doit être fourni car non géré par défaut");
            }
    ...
    private static Float calculImpots2(final float quotientFam, final MultiKeyMap baremeAnnee, final float partFiscale) {
        ...
        while (mapIterator.hasNext()) {
            ...
            float taux = (Float)next.getKey(2);

            if ((quotientFam >= min) && (quotientFam <= max) || (quotientFam >= max)) {
                float coeff = (Float)baremeAnnee.get(min, max, taux);

                float calc = add(quotientFam * taux * partFiscale, -coeff * partFiscale);
    ...
    /**
     * Ajoute les deux flaot f1 et f2 donnés en paramètre en utilisant BigDecimal afin d'éviter le problème connu de Java dans les additions
     * de float et double
     * 
     * @param f1
     * @param f2
    ...
    /**
     * Retourne null si aucun barême pour l'année anne n'est défini. Retourne le barême correspondant si l'année est définie
     * 
     * @param annee
     * @return MultiKeyMap
     */
    ...
    public static void main(final String[] args) {
        ...
        System.out.println("Impôt de l'année " + annee + " pour le revenu " + revenu + " avec quotiont familial " + part + " est de : "
                + impot);
    }
}
